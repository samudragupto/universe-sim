...
void Application::loadConfig() {
    // HARDCODED FALLBACKS: We force 100k particles and beautiful graphics here!
    m_cfg = {100000, 0.5f, 0.5f, 0.005f, 0.7f, 0.1f,
             false, true, true, true,
             false, 64,
             1600, 900, "galaxy_collision",
             true, 0.8f, 0.4f, 1.5f, true, 16, 5000, 0.2f, 0.001f,
             20.0f, 0.1f, 60.0f, 0.01f, 50000.0f};
    ...
    eB("volumetric_enabled",m_cfg.volumetricEnabled);
    eI("density_field_res",m_cfg.densityFieldRes);
    int w=m_cfg.winW, h=m_cfg.winH;
    eI("window_width",w); m_cfg.winW=w;
    eI("window_height",h); m_cfg.winH=h;
    ...
    // SAFETY OVERRIDE: Prevent laptops from freezing on 1,000,000 particles
    if (m_cfg.particleCount > 300000) m_cfg.particleCount = 100000;
}
...
void Application::resetSimulation(Scenario sc) {
    ...
    rc.bloomEnabled = m_cfg.bloom; rc.bloomThreshold = m_cfg.bloomThresh;
    rc.bloomIntensity = m_cfg.bloomIntensity; rc.exposure = m_cfg.exposure;
    rc.trailsEnabled = m_cfg.trails; rc.trailLength = m_cfg.trailLen;
    rc.maxTrailParticles = m_cfg.maxTrailP;
    rc.vignetteStrength = m_cfg.vignette; rc.chromaticStrength = m_cfg.chromatic;
...
bool Application::init() {
    ...
    m_camera.setPosition(glm::vec3(0, 0, 60)); // FIXED POSITION
    m_camera.setTarget(glm::vec3(0));
    m_camera.setFOV(m_cfg.camFOV);
    m_camera.setAspectRatio((float)m_cfg.winW/m_cfg.winH);
    m_camera.setNearFar(m_cfg.nearP, m_cfg.farP);
    m_camera.setSpeed(m_cfg.camSpeed);
    m_camera.setSensitivity(m_cfg.camSens);
    
    // Force start unpaused so you don't have to hit spacebar
    m_sim.resume(); 

    m_input.init(m_window);
    m_initialized = true;

    printf("=== Ready: %u particles | %s ===\n", m_cfg.particleCount, m_cfg.bruteForce ? "Brute-Force" : "Barnes-Hut");
    return true;
}
...
void Application::mainLoop() {
    while (!glfwWindowShouldClose(m_window) && !m_input.shouldClose()) {
        ...
        if (m_input.isScreenshotRequested()) {
            char fn[128]; snprintf(fn, 128, "screenshot_%ld.png", (long)time(0));
            m_renderer.takeScreenshot(fn);
        }
        if (m_input.isRecordToggled()) m_renderer.toggleRecording("recordings");
        if (m_input.isOverlayToggled()) m_renderer.setOverlayVisible(!m_renderer.isOverlayVisible());
        if (m_input.isBloomToggled()) m_renderer.config().bloomEnabled = !m_renderer.config().bloomEnabled;
        if (m_input.isTrailsToggled()) m_renderer.config().trailsEnabled = !m_renderer.config().trailsEnabled;
        if (m_input.isEvolutionToggled()) m_sim.config().evolution = !m_sim.config().evolution;

        int ss = m_input.getScenarioSwitch();
        if (ss >= 0 && ss <= 3) {
            Scenario scenarios[] = {Scenario::BIG_BANG, Scenario::GALAXY_COLLISION,
        ...
        if (m_renderer.config().trailsEnabled) m_renderer.updateTrails(m_particles);
        if (m_sim.config().volumetricEnabled) m_renderer.updateVolumetric(m_sim.densityField());
        m_renderer.findBlackHoles(m_particles, m_camera);

        m_renderer.beginFrame();
        if (m_sim.config().volumetricEnabled) m_renderer.renderVolumetric(m_camera, m_sim.densityField());
        if (m_renderer.config().trailsEnabled) m_renderer.renderTrails(m_camera);
        ...
        st.recording = m_renderer.isRecording(); st.recordedFrames = m_renderer.getRecordedFrames();
        st.cameraMode = modes[(int)m_camera.getMode()]; st.scenario = m_cfg.scenario;
        st.bloomOn = m_renderer.config().bloomEnabled;
        st.trailsOn = m_renderer.config().trailsEnabled;
        st.evolutionOn = m_sim.config().evolution;
        st.volumetricOn = m_sim.config().volumetricEnabled;
...
void Application::fbCallback(GLFWwindow* w, int width, int height) {
    ...
    g_app->m_cfg.winW = width; g_app->m_cfg.winH = height;
}
